...
import { AddUpdatePageComponent } from './add-update-page/add-update-page.component';
...
export class PagesComponent {
  ...
  constructor(public sharedservice: SharedService, private pageservice: PageService, private modalService: NgbModal) {}
  ...
  getDataList(source: 'search' | 'page' | 'limit' | 'refresh' = 'refresh') {
    ...
    this.pageservice.getAllPagesByPage(this.page, this.limit, this.searchTxt).pipe(
      finalize(() => {
  ...
  addUpdateData(isedit: boolean, data?) {
    const modalRef = this.modalService.open(AddUpdatePageComponent, {
      size: 'lg',
      backdrop: 'static',
      centered: true
    });
